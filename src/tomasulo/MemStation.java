...
public class MemStation extends ReservationStation{
    ...
    public void scheduleInstruction(Instruction op, RegisterFiles reg_in, int cycles){
         ...
         addr_comp[0] = reg_in.getRegister( op.getS1() );
         addr_comp[1] = reg_in.getRegister(  op.getS2() );
         addr_comp[2] = reg_in.getRegister(  op.getDes() ); // the register being read-store or written-load
    ...
    private void setStore(){
         is_store = isStore( instruction.getOper() );
    }
    ...
    public boolean hasPriority( HashMap<String, Integer> memory_buffer ){
         ...
         if( memory_buffer.containsKey( result )){              
              if( memory_buffer.get( result ).intValue() >= ( instruction.getIssueNum() ) ){
                   to_return = true;
              }
         }    
         else{
              if( instruction != null ){
                   memory_buffer.put( result, new Integer( instruction.getIssueNum() ) );
                   to_return = true;
              }
         }
